...
const LazyMdAddIcCall = memo(() => {
  ...
  useEffect(() => {
    ...
    import("react-icons/md").then((mod) => {
      if (mounted) setIcon(() => mod.MdAddIcCall);
    });
    return () => { mounted = false; };
  }, []);
  return Icon ? <Icon /> : <span style={{ width: 24, height: 24, display: 'inline-block' }} />;
});
LazyMdAddIcCall.displayName = 'LazyMdAddIcCall';

const LazyArrowForward = memo(() => {
  const [Icon, setIcon] = useState(null);
  useEffect(() => {
    let mounted = true;
    import("react-icons/io").then((mod) => {
      if (mounted) setIcon(() => mod.IoIosArrowRoundForward);
    });
    return () => { mounted = false; };
  }, []);
  return Icon ? <Icon /> : <span style={{ width: 24, height: 24, display: 'inline-block' }} />;
...
// IconBox with intersection observer for deferred image loading
const IconBox = memo(({ icon, title }) => {
  ...
  return (
    <div className="cs_iconbox cs_style_1" ref={ref}>
      <div className="cs_iconbox_icon cs_center" style={{ minWidth: 48, minHeight: 48 }}>
        {isVisible && (
          <img
            src={`${import.meta.env.BASE_URL}img/icons/${icon}.svg`}
            alt=""
            width={48}
            height={48}
            loading="lazy"
      ...
      <h3 className="cs_iconbox_title cs_fs_24 cs_semibold">{title}</h3>
    </div>
  );
});
IconBox.displayName = 'IconBox';

// Inline SVG for tool icon - prevents render blocking
...
const iconBoxData = [
  { icon: "routinemaintenance", title: "Routine Maintenance" },
  { icon: "fastefficientrepair", title: "Fast Efficient Repair Service" }
];

// Deferred shapes - loads after main content
        ...
        <div className="row cs_gap_y_40 align-items-center">
          <div className="col-xl-6">
            <div className="cs_about_thumbnail_wrapper position-relative">
              ...
              {/* Secondary images - lazy load when in view */}
              {isInView && (
                <>
