...
const BeforeAfter = ({ title, subTitle, bgImg, beforeImg, afterTitle, afterImg, beforeTitle }) => {
  ...
  useEffect(() => {
    ...
    const images = [
      `${import.meta.env.BASE_URL}${bgImg}`,
      `${import.meta.env.BASE_URL}${beforeImg}`,
      `${import.meta.env.BASE_URL}${afterImg}`
    ];
  ...
  }, [isInView, bgImg, beforeImg, afterImg]);

  // Slider functionality using refs (no DOM queries)
          ...
          {isInView && imagesLoaded ? (
            <div className="cs_before_after" ref={containerRef}>
              <div
                ...
                style={{ backgroundImage: `url(${import.meta.env.BASE_URL}${beforeImg})` }}
              >
